...
<body class="container mt-5">
    ...
    <form method="POST"
        action="/mini-erp/public/index.php?rota=produtos&acao=<?= isset($produto_para_edicao) && !empty($produto_para_edicao) ? 'atualizar' : 'salvar' ?>"
        class="mb-5">
        <input type="hidden" name="id"
            value="<?= isset($produto_para_edicao['id']) ? htmlspecialchars($produto_para_edicao['id']) : '' ?>">
        ...
        <button type="button" id="add-variacao" class="btn btn-outline-primary btn-sm mb-3">+ Adicionar
            variação</button>
        <br>
    ...
    <a href="/mini-erp/public/index.php?rota=pedidos&acao=listar" class="btn btn-secondary mb-4">Ver Todos os
        Pedidos</a>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>—</td>
                <td>
                    <a href="/mini-erp/public/index.php?rota=carrinho&acao=adicionar&id=<?= $p['id'] ?>"
                        class="btn btn-info btn-sm">Comprar</a>
                    <a href="/mini-erp/public/index.php?rota=produtos&acao=editar&id=<?= $p['id'] ?>"
                        class="btn btn-primary btn-sm">Editar</a>
                    <a href="/mini-erp/public/index.php?rota=produtos&acao=excluir&id=<?= $p['id'] ?>"
                        class="btn btn-danger btn-sm"
                        onclick="return confirm('Tem certeza que deseja excluir este produto e todas as suas variações de estoque?');">Excluir</a>
                </td>
            </tr>
            <?php else: ?>
